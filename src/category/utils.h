...
template <class Cat> constexpr bool is_category() {
  using C = std::decay_t<Cat>;
  return std::is_base_of_v<Category, C>;
};

template <class Cat> constexpr bool is_category(Cat const &) {
  return is_category<Cat>();
};

template <class X> constexpr bool has_category() {
...
template <class X> constexpr bool has_category(X const &) {
  return has_category<X>();
}

template <class X, class Cat> constexpr bool in_category() {
  using DX = std::decay_t<X>;
  using C  = std::decay_t<Cat>;
...
template <class Obj> constexpr bool is_object() {
...
template <class Obj> constexpr bool is_object(Obj const &obj) {
  return is_object<Obj>();
}

template <class Morph> constexpr bool is_morphism() {
...
template <class Morph> constexpr bool is_morphism(Morph const &) {
  return is_morphism<Morph>();
}

template <class X, class Y> constexpr bool in_same_category() {
  using DX = std::decay_t<X>;
...
template <class Morph1, class Morph2> constexpr bool has_same_source() {
...
template <class Morph1, class Morph2> constexpr bool has_same_target() {
  using M1 = std::decay_t<Morph1>;
  using M2 = std::decay_t<Morph2>;
  if constexpr (!is_morphism<M1>() || !is_morphism<M2>()) {
    return false;
...
template <class Morph1, class Morph2> constexpr bool in_same_hom_set() {
...
template <class MorphSnd, class MorphFst> constexpr bool are_composable() {
...
template <class Morph> struct morphism_traits {

  static constexpr bool is_result_of_operation = false;
};
